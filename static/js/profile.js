function setupProfileEditing() {
    ...
    if (bioEditBtn && bioOverlay && bioForm && bioContent && cancelBioBtn) {
    ...
    // Check if all required elements exist before adding event listeners
    if (picEditBtn && picOverlay && picForm && cancelPicBtn) {
    ...
    // Close overlays when clicking outside (only if overlays exist)
    if (bioOverlay && picOverlay) {
...
// Profile deletion functionality - move inside the DOM loaded event handler
document.addEventListener('DOMContentLoaded', function () {
    setupProfileEditing();

    // Get CSRF token again (since we need it outside the setupProfileEditing function)
    function getCookie(name) {
        let cookieValue = null;
        if (document.cookie && document.cookie !== '') {
            const cookies = document.cookie.split(';');
